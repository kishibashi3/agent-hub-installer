...
├── .env                     # AGENT_HUB_GITHUB_PAT 等 secret (chmod 600)
...
│   └── bridge-claude.pid    # bridge spawn 時の PID (Step 5 で書き込み, bridge stop で使用)
...
### `state/bridge-claude.pid` — bridge プロセス PID

install flow Step 5 の bridge spawn 時に `echo $! > ~/.agent-hub/state/bridge-claude.pid` で書き込む。`hubctl bridge stop` が参照して bridge プロセスを終了する。
