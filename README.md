...
```bash
# macOS (BSD sed) / Linux (GNU sed) 双方で動くよう perl -i を使用
# (BSD sed は -i に拡張子引数が必須・\b 未サポートのため sed -i は非可搬)

# 1. env.sh を更新 (一括置換)
perl -i -pe '
  s/\bGITHUB_PAT\b/AGENT_HUB_GITHUB_PAT/g;
  s/\bDB_PATH\b/AGENT_HUB_DB_PATH/g;
  s/\bAUTH_MODE\b/AGENT_HUB_AUTH_MODE/g;
' ~/.agent-hub/env.sh

# 2. self-host の場合は .env も更新 (行頭の変数名のみ置換)
perl -i -pe '
  s/^GITHUB_PAT=/AGENT_HUB_GITHUB_PAT=/;
  s/^DB_PATH=/AGENT_HUB_DB_PATH=/;
  s/^AUTH_MODE=/AGENT_HUB_AUTH_MODE=/;
' .env
