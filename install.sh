...
probe_auth_mode() {
  ...
  if command -v python3 >/dev/null 2>&1; then
    _auth_mode=$(printf '%s' "${_response}" \
      | python3 -c "import sys,json; d=json.load(sys.stdin); print(d.get('auth_mode',''))" \
      2>/dev/null || true)
  fi
  if [[ -z "${_auth_mode}" ]]; then
    _auth_mode=$(printf '%s' "${_response}" \
      | sed -nE 's/.*"auth_mode"[[:space:]]*:[[:space:]]*"([^"]+)".*/\1/p' \
      | head -1 || true)
  fi
...
# ============================================================
# agenthubctl install (issue #48)
# ============================================================

install_agenthubctl() {
  # Install agenthubctl CLI to ~/.local/bin/ (issue #48).
  # Provides: init / start / doctor subcommands as PE entry point.
  # Also caches this installer to ~/.agent-hub/install.sh for delegation.
  # agenthubctl は常に (再)書き込み — env.sh / .env (user data) と異なり managed binary なので
  # reinstall 時にも最新版に更新する (Minor #3: cache update との対称性)。
  local bin_dir="${HOME}/.local/bin"
  local ctl_bin="${bin_dir}/agenthubctl"
  local cached_installer="${AGENT_HUB_DIR}/install.sh"

  info "Installing agenthubctl..."

  # Cache installer for delegation (best effort: copy if $0 looks like this installer).
  # Running via `curl | bash` means $0 = bash → grep guard prevents wrong copy.
  if [[ "${DRY_RUN}" == "yes" ]]; then
    c_dim "[dry-run] would cache installer to ${cached_installer}"
  else
    local _script
    _script=$(realpath "$0" 2>/dev/null || readlink -f "$0" 2>/dev/null || echo "")
    if [[ -f "${_script}" ]] && grep -q 'agent-hub installer' "${_script}" 2>/dev/null; then
      cp "${_script}" "${cached_installer}" && chmod 755 "${cached_installer}" || true
      c_dim "  Cached installer → ${cached_installer}"
    else
      c_dim "  Running via pipe — cache will be populated on first agenthubctl init/doctor use"
    fi
  fi

  # Write agenthubctl binary (always — keeps in sync with cached installer version).
  info "Writing agenthubctl to ${ctl_bin}..."
  if [[ "${DRY_RUN}" == "yes" ]]; then
    c_dim "[dry-run] would create ${bin_dir}/"
    c_dim "[dry-run] would write ${ctl_bin} (init / start / doctor)"
    return
  fi

  mkdir -p "${bin_dir}"

  # Quoted heredoc: all content is written literally; runtime variables are expanded
  # when the generated script runs, not at install time. No $(cmd) or ${var} expansion here.
  cat > "${ctl_bin}" <<'AGENTHUBCTL_EOF'
#!/usr/bin/env bash
# agenthubctl — agent-hub PE entry command (issue #48)
# Generated by agent-hub installer.  To regenerate: bash ~/.agent-hub/install.sh
set -euo pipefail

AGENT_HUB_DIR="${AGENT_HUB_DIR:-${HOME}/.agent-hub}"   # runtime default — override by setting AGENT_HUB_DIR before running
ENV_SH="${AGENT_HUB_DIR}/env.sh"
CACHED_INSTALLER="${AGENT_HUB_DIR}/install.sh"
INSTALLER_URL="https://kishibashi3.github.io/agent-hub-installer/install.sh"
...
_AGENTHUBCTL_TMP=""   # global: temp file path for EXIT trap cleanup (Minor #4)

_run_installer() {
  # Run a subcommand via the cached installer; download on demand if not found.
  # Critical #1: download only after explicit user confirmation (prevents silent
  # code execution from a potentially compromised CDN/network path).
  local _subcmd="$1"; shift
  if [[ -x "${CACHED_INSTALLER}" ]]; then
    exec bash "${CACHED_INSTALLER}" "${_subcmd}" "$@"
  fi
  # Cached installer not found — require user confirmation before downloading.
  warn "Cached installer not found at ${CACHED_INSTALLER}."
  if [[ -t 0 ]]; then
    # Interactive terminal: show URL and ask for explicit consent
    warn "Will download from: ${INSTALLER_URL}"
    printf '[warn]  Proceed with download? [y/N] ' >&2
    local _confirm
    read -r _confirm
    case "${_confirm}" in
      y|Y|yes|YES) ;;
      *) die "Aborted. To restore cache, re-run: curl -fsSL ${INSTALLER_URL} | bash" ;;
    esac
  else
    # Non-interactive: fail safe — never auto-execute downloaded code without consent
    die "stdin is not a terminal. Re-run: curl -fsSL ${INSTALLER_URL} | bash"
  fi
  _AGENTHUBCTL_TMP=$(mktemp /tmp/agenthubctl-XXXXXX.sh 2>/dev/null || printf '/tmp/agenthubctl-%s.sh' "$$")
  trap 'rm -f "${_AGENTHUBCTL_TMP}"' EXIT   # Minor #4: cleanup on any exit path
  if ! curl -fsSL "${INSTALLER_URL}" -o "${_AGENTHUBCTL_TMP}" 2>/dev/null; then
    die "Cannot download installer."
  fi
  chmod +x "${_AGENTHUBCTL_TMP}"
  # Cache for future invocations
  mkdir -p "$(dirname "${CACHED_INSTALLER}")" \
    && cp "${_AGENTHUBCTL_TMP}" "${CACHED_INSTALLER}" && chmod 755 "${CACHED_INSTALLER}" 2>/dev/null || true
  bash "${_AGENTHUBCTL_TMP}" "${_subcmd}" "$@"
  exit $?   # trap handles _AGENTHUBCTL_TMP cleanup on exit
}

cmd_init() {
  # Delegate to install.sh generate-env (issue #46).
  _run_installer generate-env "$@"
}

cmd_start() {
  # Source env.sh and launch claude in operator workdir.
  if [[ ! -f "${ENV_SH}" ]]; then
    err "env.sh not found at ${ENV_SH}."
    err "  Run first: agenthubctl init --user <handle>"
    exit 1
  fi
  ...
  if ! command -v claude >/dev/null 2>&1; then
    die "claude command not found. Install Claude Code: https://claude.ai/download"
  fi

  # Determine launch dir: prefer operator workdir (AGENT_HUB_ROLES/operator/).
  # Claude Code auto-picks up CLAUDE.md from the working directory on startup.
  local _launch_dir
  _launch_dir="$(pwd)"
  if [[ -n "${AGENT_HUB_ROLES:-}" ]] && [[ -f "${AGENT_HUB_ROLES}/operator/CLAUDE.md" ]]; then
    _launch_dir="${AGENT_HUB_ROLES}/operator"
    info "Launching claude in operator workdir: ${_launch_dir}"
  elif [[ -n "${AGENT_HUB_ROLES:-}" ]]; then
    warn "operator/CLAUDE.md not found at ${AGENT_HUB_ROLES}/operator/CLAUDE.md"
    warn "  Launching claude from current directory."
  fi

  ok "Starting claude as operator ✅"
  cd "${_launch_dir}"
  exec claude
}

cmd_doctor() {
  # Delegate to install.sh doctor (issue #37).
  _run_installer doctor "$@"
}

usage() {
  cat <<'USAGE_EOF'
agenthubctl — agent-hub PE entry command

USAGE:
  agenthubctl <subcommand> [options]

SUBCOMMANDS:
  init [--user <handle>] [--hub-url <url>] [--dry-run]
                         ~/.agent-hub/env.sh を生成し ~/.bashrc / ~/.zshrc に source 行を追記
                         (= install.sh generate-env に委譲)
  start                  env.sh を読んで operator persona で claude を起動
                         AGENT_HUB_ROLES/operator/CLAUDE.md を workdir として使用
  doctor                 環境チェック (= install.sh doctor に委譲)
  help, -h, --help       このメッセージ

EXAMPLES:
  agenthubctl init --user mybot
  agenthubctl init --user mybot --hub-url https://agent-hub-ki.fly.dev/mcp
  agenthubctl start
  agenthubctl doctor
USAGE_EOF
}
...
case "${1:-}" in
  ...
  "")             usage; exit 1 ;;
  *)              err "unknown subcommand: $1 (see: agenthubctl help)"; exit 1 ;;
esac
AGENTHUBCTL_EOF

  chmod +x "${ctl_bin}"
  ok "agenthubctl installed (${ctl_bin}) ✅"
...
main() {
  ...
  install_agenthubctl     # agenthubctl CLI を ~/.local/bin/ に配置 (issue #48)
