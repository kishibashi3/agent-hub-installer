...
install_agenthubctl() {
  ...
  # Idempotent: skips agenthubctl write if already installed.
  ...
  # Write agenthubctl binary (idempotent: skip if already installed)
  if [[ -f "${ctl_bin}" ]]; then
    info "agenthubctl exists at ${ctl_bin}, preserving"
    return
  fi
...
_run_installer() {
  ...
  warn "Downloading from ${INSTALLER_URL}..."
  local _tmp
  _tmp=$(mktemp /tmp/agenthubctl-XXXXXX.sh 2>/dev/null || printf '/tmp/agenthubctl-%s.sh' "$$")
  if ! curl -fsSL "${INSTALLER_URL}" -o "${_tmp}" 2>/dev/null; then
    rm -f "${_tmp}"
    die "Cannot download installer. Re-run: curl -fsSL ${INSTALLER_URL} | bash"
  fi
  chmod +x "${_tmp}"
  # Cache for future invocations
  mkdir -p "$(dirname "${CACHED_INSTALLER}")" \
    && cp "${_tmp}" "${CACHED_INSTALLER}" && chmod 755 "${CACHED_INSTALLER}" 2>/dev/null || true
  bash "${_tmp}" "${_subcmd}" "$@"
  local _rc=$?
  rm -f "${_tmp}"
  exit "${_rc}"
